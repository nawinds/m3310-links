# Сайт полезных ссылок группы M3310 Университета ИТМО — [M3310.nawinds.dev](https://m3310.nawinds.dev)

### Useful links website for M3310 group @ ITMO University — [M3310.nawinds.dev](https://m3310.nawinds.dev)
...
Категории уведомлений реализованы с помощью OneSignal Data Tags. Сайт автоматически записывает выбранные пользователем значения:

| Категория | OneSignal tag | Значение подписки |
|---|---|---|
| Учебные материалы | `m3300_notify_study` | `1` |
| Дедлайны | `m3300_notify_deadlines` | `1` |
| Улучшения сайта | `m3300_notify_other` | `1` |

Дополнительная настройка OneSignal для работы workflow не требуется: Actions отправляет сообщения напрямую по фильтру соответствующего тега. Для просмотра аудитории каждой категории можно создать необязательные сегменты в **Audience → Segments → New Segment**. Для каждого сегмента добавьте фильтр **User Tag**, выберите ключ из таблицы и условие **equals `1`**.

После успешной публикации workflow отправляет уведомления по следующим правилам, перечисленным в порядке приоритета:

| Условие коммита | Категория уведомления |
|---|---|
| Первая строка начинается с `!other` | Улучшения сайта, даже если изменён только файл учебных материалов или дедлайнов |
| Первая строка начинается с `!study` | Учебные материалы |
| Первая строка начинается с `!deadlines` | Дедлайны |
| Изменён ровно один файл, и он находится в `src/pages/` | Учебные материалы |
| Изменён только `static/DEADLINES.json` | Дедлайны |
| Любой другой коммит | Улучшения сайта |

Префикс категории удаляется из отображаемого текста уведомления. Если после него указаны две непустые строки, первая становится заголовком, а вторая — сообщением. Если строка одна, она становится сообщением, а заголовок зависит от категории: «Новые материалы», «Новые дедлайны» или «Улучшение сайта».

Примеры сообщений коммитов:

```text
!study Добавлены материалы по математике
```

```text
!deadlines Обновлены сроки сдачи
Добавлен дедлайн третьей лабораторной работы
```

```text
!other Улучшена навигация
```

Если сообщение коммита начинается с `#`, уведомление для него не отправляется.
